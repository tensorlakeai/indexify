...
/// Determines if the build directory needs to be rebuilt based on file
/// modification times or existence.
fn needs_rebuild(source_dir: &str, build_dir: &str) -> Result<bool, Box<dyn Error>> {
    ...
    let build_time = match build_path.metadata()?.modified() {
        Ok(time) => time,
        Err(_) => return Ok(true), /* If we can't get the modification time, assume rebuild is
                                    * needed */
    };

    let node_modules_path = Path::new(source_dir).join("node_modules");

    // Recursively check if any file in the source directory is newer than the build
