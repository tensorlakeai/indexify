...
//         super::Transaction::RocksDB(Arc::new(RocksDBTransaction { db: self, tx }))
//     }
...
//     fn drop(&mut self, cf: &str) -> Result<(), DriverError> {
//         self.db.drop_cf(cf).map_err(Error::into_generic)
//     }

//     fn create(&mut self, name: &str, opts: &super::CreateOptions) -> Result<(), DriverError> {
//         let super::CreateOptions::RocksDB(opts) = opts.clone() else {
//             todo!()
//         };
...
//         let super::IterOptions::RocksDB((opts, mode)) = options else {
//             todo!()
//         };
//         let mode = mode.unwrap_or(IteratorMode::Start);

//         let iter = self.db.iterator_cf_opt(self.column_family(cf), opts, mode);

//         Box::new(iter.map(|item| item.map_err(Error::into_generic)))
//     }
// }
...
//     pub async fn get(&self, table: &str, key: &[u8]) -> Result<Option<Vec<u8>>, DriverError> {
//         let attrs = &[KeyValue::new("driver", "rocksdb")];
