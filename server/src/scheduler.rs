...
mod tests {
    ...
    use data_model::{
        test_objects::tests::{
            mock_executor, mock_executor_id, mock_graph_with_reducer, mock_invocation_payload,
            mock_invocation_payload_graph_b, mock_node_fn_output, reducer_fn, test_compute_fn,
            TEST_EXECUTOR_ID, TEST_NAMESPACE,
        },
        ComputeGraphCode, DataPayload, ExecutorId, GraphVersion, Node, RuntimeInformation, Task,
        TaskId, TaskOutcome,
    };
    use data_model::{ComputeGraph, InvocationPayloadBuilder};
    use state_store::{
    ...
    fn make_finalize_request(
        ...
        let node_outputs = (0..num_outputs)
            .map(|_| mock_node_fn_output(invocation_id, compute_graph, compute_fn_name, None))
            .collect();
        FinalizeTaskRequest {
    ...
    // TODO write edge case test case when all fn_map finish state changes are handled in the same runloop of the executor!
    ...
    async fn test_reducer_graph() -> Result<()> {
        ...
        */
        ...
        pending_tasks.iter().for_each(|t| {
            assert_eq!(t.compute_fn_name, "fn_map");
        });
        ...
        for _ in 0..3 {
            ...
            assert_eq!(pending_task.compute_fn_name, "fn_reduce");
