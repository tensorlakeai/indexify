...
async fn create_invocation_event_stream(
    ...
    async_stream::stream! {
        ...
        {
            Ok(Some(invocation)) if invocation.completed => {
                ...
                match rx.recv().await {
                    ...
                    Err(RecvError::Lagged(num)) => {
                        ...
                        match state
                            ...
                            .invocation_ctx(namespace.as_str(), compute_graph.as_str(), &id)
                        {
                            Ok(Some(context)) if context.completed => {
                                yield Event::default().json_data(
                                    InvocationStateChangeEvent::InvocationFinished(
                                        InvocationFinishedEvent {
                                            id: id.clone()
                            ...
                            _ => {} // Continue monitoring if not completed
                        }
                    }
...
pub async fn wait_until_invocation_completed(
    ...
    let invocation_event_stream = async_stream::stream! {
        // Check completion before reading from task event stream in case it was previously completed.
        // This is done after subscribing to the task event stream to avoid missing the completion event.
        match state
            .indexify_state
            .reader()
            .invocation_ctx(&namespace, &compute_graph, &invocation_id)
        {
            Ok(Some(invocation)) => {
                ...
                // Not completed, continue to stream
            }
            Ok(None) => {
                info!("invocation not found, stopping stream");
                return;
