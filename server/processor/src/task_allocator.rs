...
use data_model::{
    Allocation, AllocationBuilder, ChangeType, ExecutorId, FunctionExecutor, FunctionExecutorId,
    FunctionExecutorStatus, Task, TaskId, TaskStatus,
};
...
impl TaskAllocationProcessor {
    ...
    pub fn reconcile_executor_state(
        ...
        // TODO: for get_desired_state, manage the lifecycle of function executors so that
        //       1- we remove any function executors that are unhealthy or non-existent
        //       2- we reallocate any tasks on a function executor being removed
    ...
    // TODO: For dev executors, we need to ensure that the function executor is passed in the SchedulerUpdateRequest
    fn get_valid_function_executors(
        ...
        for (executor_id, executor) in indexes.executors.iter() {
            ...
            // For dev executors, we need to ensure that the function executor is created in the indexes
            if executor.development_mode {
