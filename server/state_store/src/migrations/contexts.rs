...
impl PrepareContext {
    ...
    /// Open DB with specific column families
    pub fn open_db_with_cfs(&self, cfs: &[String]) -> Result<TransactionDB> {
        TransactionDB::open_cf(
            &self.db_opts,
            &TransactionDBOptions::default(),
            &self.path,
...
impl<'a> MigrationContext<'a> {
    ...
    pub fn count_entries(&self, column_family: &IndexifyObjectsColumns) -> Result<usize> {
        let mut count = 0;
        self.iterate_cf(column_family, |_, _| {
            count += 1;
            Ok(())
        })?;
        Ok(count)
