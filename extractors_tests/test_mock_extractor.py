...
from indexify_extractor_sdk.base_extractor import ExtractorWrapper, Content, ContentType
...
class TestMockExtractor(unittest.TestCase):
    ...
    def test_mock_extractor(self):
        ...
        e = MockExtractor()
        extracted_content = e.extract([Content(content_type=ContentType.Text, data="Hello World")], params)
    ...
    def test_extractor_wrapper(self):
        ...
        extracted_content = e.extract([Content(content_type=ContentType.Text, data="Hello World")], '{"a": 1, "b": "foo"}')
        self.assertEqual(len(extracted_content), 1)
        self.assertEqual(len(extracted_content[0]), 3)
