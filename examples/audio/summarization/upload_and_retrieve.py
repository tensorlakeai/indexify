...
def upload(file_url):
    ...
    content_id = client.upload_file("audio_summarization", "audio.mp3")
    return content_id

def retrieve(content_id):
    print(f"waiting for extraction of {content_id}", flush=True)
    client.wait_for_extraction(content_id)
    transcription = client.get_extracted_content(content_id, "audio_summarization", "transcription")
    print(f"transcription: \n", flush=True)
    print(transcription)
    summary = client.get_extracted_content(content_id, "audio_summarization", "summarization")
    print(summary)
