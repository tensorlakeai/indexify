...
def upload(file_url):
    ...
    content_id = client.upload_file("transcription_indexing", "audio.mp3")
    return content_id

def retrieve(content_id):
    print(f"waiting for extraction of {content_id}", flush=True)
    client.wait_for_extraction(content_id)
    transcription = client.get_extracted_content(content_id, "transcription_indexing", "transcription")
...
if __name__ == "__main__":
    ...
    retrieve(content_id)
    os.remove("audio.mp3")
    results = client.search_indexes("transcription_indexing.embedding", "black hole", 3)
    print(results)