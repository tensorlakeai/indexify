...
from inkwell.layout_detector import LayoutDetectorType, LayoutDetectorFactory
from inkwell.table_extractor import TableExtractorType, TableExtractorFactory 
...
class PDFParser(IndexifyFunction):
    ...
    def __init__(self):
        super().__init__()
        self.setup_local_pipeline()

    def setup_local_pipeline(self):


        # Download the models by cloning the repositories and add the appropriate paths
        # https://huggingface.co/tensorlake/faster_rcnn-publaynet
        # https://huggingface.co/microsoft/Phi-3.5-vision-instruct
        
        layout_detector = LayoutDetectorFactory.get_layout_detector(
            LayoutDetectorType.FASTER_RCNN,
            model_path="/home/ubuntu/Faster-RCNN",
            **{"detection_threshold": 0.4}
        )

        table_extractor = TableExtractorFactory.get_table_extractor(
            TableExtractorType.PHI3_VISION,
            model_path="/home/ubuntu/Phi-3.5-vision-instruct"
        )
