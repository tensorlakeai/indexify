...
export async function ExtractionGraphsPageLoader({ params }: LoaderFunctionArgs) {
  if (!params.namespace) return redirect("/");
  const client = await createClient(params.namespace);
...
export async function ExtractionPolicyPageLoader({ params }: LoaderFunctionArgs) {
...
export async function IndexesPageLoader({ params }: LoaderFunctionArgs) {
  ...
  return { indexes, namespace: params.namespace, extractionGraphs: client.extractionGraphs };
}

export async function IndividualContentPageLoader({ params }: LoaderFunctionArgs) {
  ...
  const extractedMetadataList = await client
    ...
    .catch((e) => {
      if (isAxiosError(e)) {
        errors.push(`getExtractedMetadata: ${e.message} - ${e.response?.data || 'unknown'}`);
      }
...
export async function SearchIndexPageLoader({ params }: LoaderFunctionArgs) {
  ...
  const client = await createClient(namespace);
  const indexes = (await client.indexes()).filter(index => index.name === indexName);
  if (!indexes.length) {
    return redirect('/');
  }
