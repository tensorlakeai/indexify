...
impl ExampleClient {
    ...
    ///
    /// The request will be processed by raft protocol: it will be replicated to
    /// a quorum and then will be applied to state machine.
    ///
    ...
    ///
    /// This method may return stale value because it does not force to read on
    /// a legal leader.
    ...
    ///
    /// The node to add has to exist, i.e., being added with
    ...
    ///
    /// All nodes in `req` have to be already added as learner with
    /// [`add_learner`], or an error [`LearnerNotFound`] will be returned.
    ...
    ///
    /// Metrics contains various information about the cluster, such as current
    /// leader, membership config, replication status etc.
    /// See [`RaftMetrics`].
    ...
    /// returned and this client will retry at most 3 times to contact the
    /// updated leader.
