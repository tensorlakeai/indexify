...
use crate::{
    internal_api::{ContentMetadata, ExtractionEvent, ExtractorBinding, Task},
...
impl App {
    ...
    pub async fn get_executors_for_extractor(&self, extractor: &str) -> Result<Vec<String>> {
        ...
        let executors = store
            .extractors_table
            .get(extractor)
            .map(|extractors| extractors.clone())
            .unwrap_or(vec![]);
        Ok(executors)
    }
    ...
    pub async fn get_conent_metadata(&self, content_id: &str) -> Result<ContentMetadata> {
        let store = self.store.state_machine.read().await;
        let content_metadata = store.content_table
            .get(content_id)
    ...
    pub async fn create_tasks(&self, tasks: Vec<Task>) -> Result<()> {
        let _resp = self.raft.client_write(Request::CreateTasks { tasks }).await?;
        Ok(())
    }

    pub async fn tasks_for_executor(&self, executor_id: &str) -> Result<Vec<Task>> {
        let store = self.store.state_machine.read().await;
        let tasks = store.task_assignments.get(executor_id).map(|task_ids| {
            task_ids
                .iter()
