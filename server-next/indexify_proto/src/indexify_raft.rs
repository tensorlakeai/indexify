...
pub mod raft_api_server {
    ...
    #[async_trait]
    pub trait RaftApi: std::marker::Send + std::marker::Sync + 'static {
        async fn forward(
            &self,
    ...
    impl<T> Clone for RaftApiServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "indexify_raft.RaftApi";
    impl<T> tonic::server::NamedService for RaftApiServer<T> {
