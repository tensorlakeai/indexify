...
class Downloader:
    ...
    async def download_graph(self, namespace: str, name: str):
        ...
        console.print(Panel(f"Downloading graph: {name}\nPath: {path}", title="Downloader", border_style="cyan"))
        ...
        try:
            ...
            console.print(Panel(f"Failed to download graph: {name}\nError: {response.text}", title="Downloader Error", border_style="error"))
            raise
    ...
    async def download_input(self, task: Task) -> IndexifyData:
        ...
        console.print(Panel(f"Downloading input\nURL: {url}", title="Downloader", border_style="cyan"))
        
        response = httpx.get(url)
        try:
            response.raise_for_status()
        except httpx.HTTPStatusError as e:
            console.print(Panel(f"Failed to download input: {task.input_key}\nError: {response.text}", title="Downloader Error", border_style="error"))
            raise
