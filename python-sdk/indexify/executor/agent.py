...
from .executor_tasks import CreateRunFunctionRequestTask, RunFunctionTask
...
class ExtractorAgent:
    ...
    async def task_launcher(self):
        ...
        fn_queue: List[Tuple[InitializeRequest, RunTaskRequest]] = []
        ...
        while True:
            for initialize_request, run_task_request in fn_queue:
                task: Task = self._task_store.get_task(run_task_request.task_id)
                async_tasks.append(
                    RunFunctionTask(
            ...
            for async_task in done:
                ...
                elif async_task.get_name() == "create_run_function_request":
                    if async_task.exception():
                        logger.error(
                            "failed creating RunTaskRequest for task",
                        ...
                        continue
                    requests: Tuple[InitializeRequest, RunTaskRequest] = (
                        await async_task
                    )
                    fn_queue.append(requests)
