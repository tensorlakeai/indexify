...
class InvocationStateProxyServer:
    ...
    def __init__(
        self, client_response_iterator: Iterator[InvocationStateResponse], logger: Any
    ):
        self._client_response_iterator: Iterator[InvocationStateResponse] = (
            client_response_iterator
        )
        self._logger: Any = logger.bind(module=__name__)
        # Python support big integers natively so we don't need
    ...
    def _reciever(self) -> None:
        for response in self._client_response_iterator:
    ...
    def set(self, task_id: str, key: str, value: str) -> None:
        with self._lock:
    ...
    def get(self, task_id: str, key: str) -> Optional[str]:
        with self._lock:
            request_id: str = str(self._request_seq_num)
            self._request_seq_num += 1
            ...
            return (
                response.get.state_value
                if response.get.HasField("state_value")
                else None
            )
