...
from indexify.functions_sdk.object_serializer import JsonSerializer
...
class ProxiedInvocationState(InvocationState):
    ...
    def __init__(self, task_id: str, proxy_server: InvocationStateProxyServer):
        ...
        self._proxy_server: InvocationStateProxyServer = server

    def set(self, key: str, value: Any) -> None:
        """Set a key-value pair."""
        self._proxy_server.set(self._task_id, key, JsonSerializer.serialize(value))

    def get(self, key: str, default: Optional[Any] = None) -> Optional[Any]:
        """Get a value by key. If the key does not exist, return the default value."""
        serialized_value: Optional[str] = self._proxy_server.get(self._task_id, key)
        return (
            default
            if serialized_value is None
            else JsonSerializer.deserialize(serialized_value)
        )
