...
def summarize_website(website_data: WebsiteData) -> SummarizeWebsite:
    ...
    response = client.chat.completions.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": "You are a helpful assistant that summarizes website content in a form which is hearable as a podcast. Remove all the marketing content and only keep the core content in the summary. Call it Tensorlake Daily, don't add words such as host, pause, etc. The transcript of the summary is going to be fed into a TTS model which will add the pauses."},
            {"role": "user", "content": f"Summarize the following content: {website_data.content}"}
        ],
        max_tokens=3000,
        temperature=0.7,
    )
...
if __name__ == "__main__":
    ...
    invocation_id = client.invoke_graph_with_object(g.name, block_until_done=True, url="https://www.cidrap.umn.edu/avian-influenza-bird-flu/h5n1-avian-flu-virus-detected-wastewater-10-texas-cities")
    output: List[Audio] = client.graph_outputs(g.name, invocation_id, fn_name="generate_tts")
    if len(output) > 0:
