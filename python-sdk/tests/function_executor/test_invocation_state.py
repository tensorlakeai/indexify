    ...
    def loop():
        for expected_request in expected_requests:
            ...
            if request.HasField("set"):
                test_case.assertEqual(request.set.task_id, expected_request.set.task_id)
                test_case.assertEqual(
                    request.set.state_key, expected_request.set.state_key
                )
                # Two different serialized objects are not equal so we need to deserialize them.
                test_case.assertEqual(
                    CloudPickleSerializer.deserialize(request.set.state_value),
                    CloudPickleSerializer.deserialize(expected_request.set.state_value),
                )
            else:
                test_case.assertEqual(request.get.task_id, expected_request.get.task_id)
                test_case.assertEqual(
                    request.get.state_key, expected_request.get.state_key
                )
...
class TestSetInvocationState(FunctionExecutorServerTestCase):
    ...
    def test_success(self):
        with self._rpc_channel() as channel:
            ...
            expected_requests = [
                InvocationStateRequest(
                    ...
                    set=SetInvocationStateRequest(
                        ...
                        state_value=CloudPickleSerializer.serialize(
                            StructuredState(
                                string="hello",
                                integer=42,
                                structured=StructuredField(
            ...
            responses = [
                InvocationStateResponse(
                    request_id="0", set=SetInvocationStateResponse(success=True)
                ),
            ]
    ...
    def test_client_failure(self):
        with self._rpc_channel() as channel:
            ...
            responses = [
                InvocationStateResponse(
                    request_id="0", set=SetInvocationStateResponse(success=False)
                ),
            ]
            client_thread = invocation_state_client_stub(
                self, stub, expected_requests, responses
        ...
        with self._rpc_channel() as channel:
            ...
            responses = [
                InvocationStateResponse(
                    request_id="0",
                    get=GetInvocationStateResponse(
                        success=False, state_key="test_state_key"
                    ),
                ),
            ]
            client_thread = invocation_state_client_stub(
