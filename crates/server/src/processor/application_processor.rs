    ...
    pub async fn handle_state_change(
        ...
        let scheduler_update = match &state_change.change_type {
            ...
            ChangeType::SnapshotSandbox(ev) => {
                ...
                let snapshot_key =
                    data_model::SnapshotKey::new(&ev.namespace, ev.snapshot_id.get());
                if let Some(snapshot) = indexes_guard.snapshots.get(&snapshot_key) {
                    scheduler_update
                        .updated_snapshots
                        .insert(snapshot_key, snapshot.as_ref().clone());
                } else {
                    warn!(
                        namespace = %ev.namespace,
                        sandbox_id = %ev.sandbox_id,
                        snapshot_id = %ev.snapshot_id,
                        "snapshot missing in in-memory state during SnapshotSandbox handling"
                    );
                }
                return Ok(StateChangeResult::SchedulerUpdate(scheduler_update));
            }
