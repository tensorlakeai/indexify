...
fn get_docker_credentials(image: &str) -> Option<DockerCredentials> {
    ...
    let server_url = format!("https://{}", registry);
    ...
    match docker_credential::get_credential(&server_url) {
        ...
        Ok(docker_credential::DockerCredential::IdentityToken(token)) => Some(DockerCredentials {
        ...
        Err(e) => {
            tracing::debug!(
                registry = %registry,
                error = ?e,
                "No Docker credentials found for registry"
            );
            None
        }
    }
